function single_tracking2(testdir, M)
% use Haar + VGG16 feature
% ------------------------------------------------
% video file setting
% ------------------------------------------------
...
videoname = tmp{3:4};
fid = fopen(fullfile(testdir, [videoname sprintf('_result_Ours[Haar+VGG16-%d].txt', M)]), 'w');
...
pos_store.feature = get_VGG_ftr(pos_store.sampleImage, imgSr, M, VGGNet, VGGconfig);
neg_store.feature = get_VGG_ftr(neg_store.sampleImage, imgSr, M, VGGNet, VGGconfig);
...
%     clear img;
    ...
    if mod(i,dctparams.freq)==0 || disappear 
        ...
        if numel(neg_sample.sampleImage.sx) > 0
            neg_sample.feature = get_VGG_ftr(neg_sample.sampleImage, imgSr, M, VGGNet, VGGconfig);
        ...
        if sgn == 0
            ...
            disappear = 0;
            detectx.feature = get_VGG_ftr(detectx.sampleImage,imgSr, M, VGGNet, VGGconfig);
    ...
    if c > -60 || i <= 10
        ...
        pos_sample.feature = get_VGG_ftr(pos_sample.sampleImage, imgSr, M, VGGNet, VGGconfig);
