function single_tracking1(testdir)
% ------------------------------------------------
% video file setting
% ------------------------------------------------
...
for i = 2:numFrames
    ...
    if mod(i,dctparams.freq)==0 || disappear 
        disp('faster_rcnn');
        [boxes, scores] = faster_rcnn_detection(single(imgSr), 0.45, opts, proposal_detection_model, rpn_net, fast_rcnn_net);
        boxes = double(boxes);
        samples.sx = boxes(:,1)';
        samples.sy = boxes(:,2)';
        ...
        if sgn == 0
            ...
            % w and h need no update
            % inertia.x = floor(inertia.rho * inertia.x);
            % inertia.y = floor(inertia.rho * inertia.y);
